...
def identify_category_and_schemes(user_message):
    ...
    If there are any relevant scheme(s) found, output the pair(s) of a) `category` and b) the associated `financial_scheme` into a
...
def generate_response_based_on_scheme_details(user_message, scheme_details):
    ...
    Do not make up your own information.
    ...
    Do not make up your own answer. Include the information in the original csv document \
    ...
    # Format the response to include categories
    detailed_response = []
    for detail in scheme_details:
        financial_scheme = detail.get('financial_scheme')
        scheme_info = f"{financial_scheme}: {detail}"
        detailed_response.append(scheme_info)
